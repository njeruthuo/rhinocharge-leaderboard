...
function CheckpointCell({
  ...
  if (start_cp === cp.point) {
    const displayTime = completeTrip
      ? (specificItem?.time ?? cp.time ?? "7:30")
      : "7:30";
    const displayDistance = completeTrip
      ? (cp?.distanceFromBase?.toFixed(3) ?? "0.000")
      : "0.000";
        ...
        <div className="w-full">
          <span className="text-stone-500 font-mono text-[11px] block leading-none text-center opacity-80">
            {new Intl.NumberFormat("en-US").format(
              cp?.distanceFromBase || cp.odometer || 0.0,
            )}
          </span>
        </div>
      )}
